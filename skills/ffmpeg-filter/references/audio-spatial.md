...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py detect --input movie.mkv
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py binaural --input movie.mkv --output bin.m4a
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py binaural --input movie.mkv --output bin.flac --sofa mit_kemar.sofa
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py upmix --input stereo.wav --output surround.ac3 --layout 5.1
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py widen --input in.wav --output wide.wav --strength 0.5
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py split --input in.mkv --output-pattern "ch_%d.wav"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py remap --input in.wav --output out.wav --map "mono:c0=0.5*c0+0.5*c1"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/spatial.py join --inputs L.wav R.wav --output stereo.wav --layout stereo
```

## Reference docs

- Read [`references/audio-spatial-filters.md`](audio-spatial-filters.md) for per-filter option tables, channel-layout names/codes, ClockWise diagrams, SOFA sources, `pan` grammar, and a recipe book.
