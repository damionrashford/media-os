...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/vfilter.py --preset scale-720p --input in.mp4 --output out.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/vfilter.py --preset watermark --input in.mp4 --watermark logo.png --output out.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/vfilter.py --preset hstack --inputs a.mp4,b.mp4 --output side.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/vfilter.py --custom --filter-string "crop=iw-200:ih:100:0,scale=-2:720" --input in.mp4 --output out.mp4
```

## Reference docs

- Read [`references/video-filter-filters.md`](video-filter-filters.md) for exhaustive option tables, expression variables, `-filter_complex` label/map rules, drawtext escaping, and 50+ recipe snippets.
