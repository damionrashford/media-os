...
Creative and restoration audio effects. `-af` chain syntax. Every recipe below is verified against the FFmpeg filters doc. For basic gain / EQ / loudnorm / tempo see the sibling `audio-filter.md` reference — this one is modulation, delay, restoration, true-peak limiting, dynamic EQ, spectral domain, and plugin hosts.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/afx.py echo --input in.wav --output out.wav --delay-ms 600 --decay 0.4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/afx.py deckick --input vinyl.wav --output clean.wav
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-filter/scripts/afx.py limit --input master.wav --output final.wav --method psy
```

## Reference docs

Read [`references/audio-fx-filters.md`](audio-fx-filters.md) for per-filter option tables, effect-type taxonomy, LADSPA/LV2 plugin discovery, and production recipe book (vinyl chain, voice chain, radio master chain).
