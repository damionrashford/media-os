# Shaka Packager — Commercial DRM Packaging
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/shaka.py gen-clearkey-keys

# Package a test stream
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/shaka.py clearkey \
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/shaka.py widevine \
...
- Read [`shaka-reference.md`](shaka-reference.md) for protection-scheme matrix, DRM UUIDs, stream descriptor syntax, manifest options, live vs VOD, license-server vendor list, PSSH structure, FAST/live recipes, validator commands.
