# MKVToolNix (mkvmerge / mkvextract / mkvpropedit / mkvinfo)
...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/mkv.py identify --input in.mkv

# Merge
python3 ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/mkv.py merge \
  --output out.mkv --inputs v.h264 a.ac3 s.srt \
  --title "My Movie" --languages eng,jpn,eng

# Split by time
python3 ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/mkv.py split-time \
  --input in.mkv --output-pattern part.mkv --times 00:10:00,00:20:00

# Flip default flag
python3 ${CLAUDE_PLUGIN_ROOT}/skills/media-package/scripts/mkv.py default-flag \
  --input in.mkv --track a2 --value 1
```

## Reference docs

- Read [`mkvtoolnix-reference.md`](mkvtoolnix-reference.md) for full split syntax,
