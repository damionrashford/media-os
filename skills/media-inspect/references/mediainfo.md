# MediaInfo — Deep Container and Stream Analysis

Deep container and stream analysis with MediaInfo (`mediainfo --Output=JSON --Full --Inform`): codec profiles and levels, bitrate ladders, GOP structure, HDR10 + HDR10+ + Dolby Vision metadata, audio stream channel descriptions, container quirks beyond ffprobe.

Scripts: use `${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py`.
Field-name crosswalk MediaInfo vs ffprobe: see `references/mediainfo-tags.md`.
...
- **Field-name crosswalk MediaInfo vs ffprobe:** → `references/mediainfo-tags.md`
...
`--Output` picks a preset. `--Inform` customizes. `%Field%` names are MediaInfo's (not ffprobe's) — see `references/mediainfo-tags.md` for the catalog.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py check
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py summary      --input in.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py json         --input in.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py field        --input in.mp4 --stream Video --name Width
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py hdr          --input in.mkv
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py codec-profile --input in.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py compare      --inputs a.mp4 b.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py netflix-check --input in.mp4
```
...
- **`%Field%` syntax uses MediaInfo's names, NOT ffprobe's.** `%Width%` not `width`. `%FrameRate%` not `r_frame_rate`. See the crosswalk in `references/mediainfo-tags.md`.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py field --input in.mp4 --stream Video --name "Format profile"
# High@L4.1
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/mediainfo.py compare --inputs src.mov encode.mp4
```
...
Solution: Check `references/mediainfo-tags.md` for the exact `%Field%` catalog per stream type.
