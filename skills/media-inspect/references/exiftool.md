# ExifTool — Read and Write Photo/Video Metadata

Read and write EXIF, IPTC, XMP, GPS, and maker-note metadata for photos and videos with ExifTool (`exiftool -g -json -P -overwrite_original`).
...
Scripts: use `${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/exif.py`.
Tag groups, format matrix, recipe book: see `references/exiftool-tags.md`.
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/exif.py write --input in.jpg --set "Artist=Alice" --set "Copyright=(c) 2026" --preserve-mtime
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/exif.py strip --input in.jpg --gps-only --preserve-mtime
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/exif.py shift-dates --input ./trip --delta "0:0:0 1:0:0"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/media-inspect/scripts/exif.py batch-rename --dir ./trip --pattern DateTimeOriginal --format "%Y%m%d_%H%M%S.%%le"
```

## Reference docs

- Read [`references/exiftool-tags.md`](exiftool-tags.md) for tag groups, format support matrix, date/GPS conventions, and the recipe book.
