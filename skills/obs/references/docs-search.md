...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py list-pages
```
...
Read [`pages.md`](pages.md) for the full catalog with use-cases.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "obs_source_create" --limit 5
```

When you know the page, scope to it (faster, less noise):

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "hotkey_register" --page reference-core-objects
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py section --page reference-core-objects --id obs_source_create
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py fetch --page reference-frontend-api
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py index
```
...
Clear: `uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py clear-cache`.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "obs_source_create" --page reference-core-objects --limit 3
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "SetCurrentProgramScene" --page obs-websocket-protocol --limit 3
```

### Example 3 — "What Frontend API functions exist for recording control?"

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "recording" --page reference-frontend-api --limit 10
```

### Example 4 — "Plugin CMake setup?"

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py fetch --page plugintemplate-readme
```

### Example 5 — "What scripting callbacks can a Python script register?"

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsdocs.py search --query "callback" --page scripting --limit 10
```
