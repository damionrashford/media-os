...
```bash
# Use the docs lookup: search --query "<RequestOrEvent>" --page obs-websocket-protocol --limit 3
```
...
If the user needs code that runs INSIDE OBS's own Python/Lua VM, use `scripting.md`. If they need a native source / output / encoder, use `plugins.md`.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py check
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py ping
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py scene-switch --scene "Starting Soon"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py record --action toggle
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py request --type GetStats
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py events --subscribe scenes,outputs
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py batch --requests-file scene-reset.json
```
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py scene-switch --scene "Wide"
```

### Example 2: Toggle mute from voice-activity detector

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py mute --input "Mic/Aux" --action toggle
```

### Example 3: Save a replay buffer highlight

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py replay-buffer --action save
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py request \
...
- [`protocol.md`](protocol.md) — OpCodes, RequestStatus (100–703), close codes (4000–4012), EventSubscriptions bitmask, RequestBatch executionType, auth pseudocode, commonly-used Requests/Events grouped by category, client snippets for Python/Node/Go/browser, handshake diagram, keepalive strategy.
...
Solution: Tools → obs-websocket Settings → Enable WebSocket server. Verify port with `uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/wsctl.py check`.
...
Solution: Verify via the docs lookup (`docs-search.md`) before sending.
