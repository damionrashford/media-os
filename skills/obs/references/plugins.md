...
**Not this skill:** writing Python/Lua scripts that run inside OBS → use `scripting.md`. Programmatically editing profile/scene-collection files → use `config.md`. Controlling a running OBS via WebSocket → use `websocket.md`.

## MANDATORY — verify APIs before quoting them

Before claiming any function signature, struct field, or macro exists, run the docs lookup:
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py scaffold \
  --name my-plugin \
  --author "Your Name" \
...
For output / encoder / service, fill `obs_output_info` / `obs_encoder_info` / `obs_service_info` and call `obs_register_output` / `obs_register_encoder` / `obs_register_service` respectively. Full field list in `references/plugins-catalog.md`.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py build --path ~/src/my-plugin
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-local --path ~/src/my-plugin
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin streamfx
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin asc
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin move
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin ndi
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin source-record
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin backtrack
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin obs-websocket
```
...
- **[`plugins-catalog.md`](plugins-catalog.md)** — full field references for `obs_source_info`, `obs_output_info`, `obs_encoder_info`, `obs_service_info`; `OBS_SOURCE_TYPE` / `output_flags` enums; properties + settings API catalog; buildspec.json schema; OBS-version ↔ ABI matrix; signing/notarization notes; debug tips.
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py build --path ~/src/obs-my-src
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-local --path ~/src/obs-my-src
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin move
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py install-community --plugin asc
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/plugins.py list-installed
```
