...
Do NOT use this skill to remote-control a running OBS — that is `websocket.md`. Do NOT use it to write a Python/Lua script that runs inside OBS — that is `scripting.md`. Do NOT use it to build a compiled C++ plugin — that is `plugins.md`.
...
NVENC / QSV / VideoToolbox use different keys — see `references/config-catalog.md`.
...
See `references/config-catalog.md` for the full catalog and platform availability.
...
```
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py check --verbose
```
...
1. See what's already there:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py check
   ```
2. Install OBS if missing:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py install --platform auto --dry-run
   ```
3. Create profile + collection from templates:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py profile create --name Streaming --template 1080p60_stream
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py collection create --name Main --template browser-overlay
   ```
4. Wire streaming destination:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py service set --profile Streaming \
     --type rtmp_custom --server rtmp://live.example.com/app --key KEY
   ```
5. Make them default:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py profile set-default --name Streaming
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py collection set-default --name Main
   ```
6. Launch OBS and verify.

## Reference docs

- Read [`config-catalog.md`](config-catalog.md) for the full `basic.ini` key list, every source-kind ID per platform, encoder JSON shape per codec, `global.ini` keys, `service.json` variants, CLI flags, and portable mode per OS.
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py profile create \
  --name Twitch1080p60 --template 1080p60_stream
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py encoder set \
  --profile Twitch1080p60 --codec x264 --bitrate 6000 --preset veryfast
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py service set \
  --profile Twitch1080p60 --type rtmp_common --server auto --key live_XXX
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py collection create \
  --name Main --template webcam
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py profile set-default --name Twitch1080p60
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py collection set-default --name Main
open -a OBS
```
...
```bash
# On machine A
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py export \
  --profile Twitch1080p60 --output twitch.zip

# Copy twitch.zip to machine B, then:
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/obsconfig.py import --archive twitch.zip
```
...
Solution: Use `jim_nvenc` (modern) or `ffmpeg_nvenc` (legacy). The JSON keys for NVENC are different from x264 — see `references/config-catalog.md`.
