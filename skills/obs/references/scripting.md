...
- **Verify an API exists:** docs lookup, `obsdocs.py search --query <name> --page scripting`
...
Run `uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/scaffold-script.py check` to auto-detect.
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/scaffold-script.py check

# 2. Create a new script from template
uv run ${CLAUDE_PLUGIN_ROOT}/skills/obs/scripts/scaffold-script.py new-python \
...
- Read [`api.md`](api.md) for the full lifecycle / properties / settings / hotkey / signal / timer reference with every enum value.
- Authoritative upstream: docs lookup → `obsdocs.py search --query <name> --page scripting`.
...
- **No Qt.** You cannot create `QDialog`, `QDockWidget`, menu items, etc. from a script. For that you need a compiled C++ plugin (see `plugins.md`).
...
Solution: check the docs lookup or consult docs.obsproject.com directly. If zero hits on `scripting`, retry without `--page` — if it only exists in reference-core, you need a compiled plugin.
