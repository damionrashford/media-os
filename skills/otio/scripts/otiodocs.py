...
# dependencies = []
# ///
"""otiodocs.py — Search and fetch OpenTimelineIO (OTIO) documentation.

Covers opentimelineio.readthedocs.io (tutorials + Python API reference) plus
the AcademySoftwareFoundation/OpenTimelineIO GitHub README and the
OpenTimelineIO-Plugins meta-package README (community adapters).

Stdlib only (urllib + html.parser). Non-interactive. Caches text-extracted
pages to ~/.cache/otio-docs/ so repeated lookups are offline-fast.

Usage:
    otiodocs.py list-pages
    otiodocs.py fetch --page tut-adapters
    otiodocs.py search --query "RationalTime" [--page api-opentime]
    otiodocs.py section --page api-schema --id opentimelineio.schema.Clip
    otiodocs.py index
    otiodocs.py clear-cache

Override cache location with OTIO_DOCS_CACHE env var.
"""
...
UA = "otio-docs-skill/1.0 (Claude Code Agent Skill)"

RTD = "https://opentimelineio.readthedocs.io/en/latest"
GH_RAW_CORE = (
    "https://raw.githubusercontent.com/AcademySoftwareFoundation/OpenTimelineIO/main"
)
GH_RAW_PLUGINS = (
    "https://raw.githubusercontent.com/OpenTimelineIO/OpenTimelineIO-Plugins/main"
)

PAGES: dict[str, str] = {
    # Tutorials (HTML)
    "tut-quickstart": f"{RTD}/tutorials/quickstart.html",
    "tut-adapters": f"{RTD}/tutorials/adapters.html",
    "tut-timeline-structure": f"{RTD}/tutorials/otio-timeline-structure.html",
    "tut-time-ranges": f"{RTD}/tutorials/time-ranges.html",
    "tut-schemadef": f"{RTD}/tutorials/write-a-schemadef-plugin.html",
    # Python API reference (HTML)
    "api-py-root": f"{RTD}/api/python/opentimelineio.html",
    "api-opentime": f"{RTD}/api/python/opentimelineio.opentime.html",
    "api-core": f"{RTD}/api/python/opentimelineio.core.html",
    "api-schema": f"{RTD}/api/python/opentimelineio.schema.html",
    "api-algorithms": f"{RTD}/api/python/opentimelineio.algorithms.html",
    "api-media-linker": f"{RTD}/api/python/opentimelineio.media_linker.html",
    "api-hooks": f"{RTD}/api/python/opentimelineio.hooks.html",
    "api-adapters-root": f"{RTD}/api/python/opentimelineio.adapters.html",
    "api-plugins-root": f"{RTD}/api/python/opentimelineio.plugins.html",
    "api-versioning": f"{RTD}/api/python/opentimelineio.versioning.html",
    # GitHub READMEs (raw Markdown)
    "github-readme": f"{GH_RAW_CORE}/README.md",
    "plugins-readme": f"{GH_RAW_PLUGINS}/README.md",
}

CACHE_DIR = Path(
    os.environ.get("OTIO_DOCS_CACHE", Path.home() / ".cache" / "otio-docs")
)
...
def load_or_fetch(page: str, *, no_cache: bool = False, verbose: bool = False) -> str:
    if page not in PAGES:
        raise ValueError(
            f"unknown page: {page!r}. Run `otiodocs.py list-pages` to see valid names."
        )
    cp = cache_path(page)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Search and fetch OpenTimelineIO documentation.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    p1 = sub.add_parser("list-pages", help="list known OTIO doc pages")
    ...
    p4.add_argument("--page", help="limit to one page; default: all pages")
