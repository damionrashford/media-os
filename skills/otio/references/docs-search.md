...
**Triggers:** look up an OTIO class, find an adapter for a NLE format, verify Python bindings, read OpenTimelineIO docs. Searches and fetches official OpenTimelineIO (OTIO) docs from opentimelineio.readthedocs.io and github.com/AcademySoftwareFoundation/OpenTimelineIO — timeline primitives (RationalTime, TimeRange, TimeTransform), containers (Timeline/Stack/Track), content (Clip/Gap/Transition), MediaReferences, tutorials, Python API, adapter catalog.
...
- Need the `otioconvert` / `otiotool` / `otiostat` CLI surface (CLIs are documented via `--help`, not readthedocs — this pulls the tutorials + Python-API pages instead).
...
Read [`references/adapters.md`](adapters.md) for the full adapter matrix + [`references/pages.md`](pages.md) for the URL catalog.
...
- **CLIs do NOT have readthedocs pages.** `otioconvert --help`, `otiocat --help`, `otiostat --help`, `otiotool --help`, and `otiopluginfo --help` are the authoritative CLI reference. This pulls the Python tutorials + API docs that ground the CLI behavior; for CLI flags run the tools with `--help`.
