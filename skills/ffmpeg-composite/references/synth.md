# Synthetic Test Media (lavfi sources)

Source skill: `ffmpeg-synth`. Triggers: generate test bars and tone, create a calibration clip, make SMPTE color bars, generate a sine tone, output silence, create a test video, produce a solid-color background, generate 1 kHz reference tone, build synthetic assets for pipeline testing. Sources: testsrc, testsrc2, smptebars, smptehdbars, rgbtestsrc, yuvtestsrc, allyuv, allrgb, colorchecker, color (solid), mandelbrot, cellauto, life, gradients, zoneplate, sine tone, anullsrc, anoisesrc.
...
- Read [`synth-sources.md`](synth-sources.md) for full per-source option tables, color name list, `aevalsrc` grammar, and a recipe gallery (broadcast cal, ABX noise, 4K 60 fps stress, identity Hald, synthetic overlays).
