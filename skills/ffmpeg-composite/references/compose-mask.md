# Advanced Compositing, Masking & Channel Operations

Source skill: `ffmpeg-compose-mask`. Triggers: composite with masks, extract or merge alpha, manipulate individual YUV/RGB/alpha planes, roto-style compositing, displacement map one video by another, remap pixels, split and recombine channels. Filters: maskedclamp, maskedmax, maskedmin, maskedmerge, maskedthreshold, maskfun, alphamerge, alphaextract, premultiply, unpremultiply, displace, hysteresis, floodfill, remap, mergeplanes, shuffleplanes, extractplanes, shuffleframes, shufflepixels, rgbashift, blend (spatial).

Advanced compositing and channel surgery. If you are keying green-screens, see `chromakey.md`. If you are grading color, see `ffmpeg-lut-grade`. This reference is for pixel-exact compositing math, alpha plumbing, and plane-level operations.
...
- [`compose-mask-filters.md`](compose-mask-filters.md) — per-filter option tables, masking mental model, pre/un-premultiply decision tree, plane-selector hex notation, recipe book (rotoscope, sky replacement, chromatic aberration, displacement warp).
