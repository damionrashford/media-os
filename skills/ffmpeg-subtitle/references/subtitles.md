...
- **Style caption fonts / colors:** → Step 3 example A + `references/subtitles-formats.md` force_style section
...
See `references/subtitles-formats.md` for the full matrix.
...
- Read [`references/subtitles-formats.md`](references/subtitles-formats.md) for the container × codec matrix, the full `force_style` parameter list, ASS color hex format, per-format feature comparison, common fontfile paths, and filter-path escaping rules.
