...
Full per-model strengths, install quirks, and language-matrix in `references/ocr-ai-languages.md`.
...
See `references/ocr-ai-languages.md` for the supported-language matrix per backend. Some languages are paddle-only (e.g. Sanskrit, Thai), some are tesseract-only (older scripts, a long tail).
...
- Read [`references/ocr-ai-languages.md`](ocr-ai-languages.md) when picking between backends for a specific language, script, or multilingual mix (English + Japanese, CJK, Arabic/Farsi, Thai, Cyrillic).
- Read [`references/ocr-ai-LICENSES.md`](ocr-ai-LICENSES.md) before shipping OCR output commercially or redistributing the model weights.
...
- **Non-Latin scripts in PaddleOCR need a different rec model.** The default `en`-only model won't recognize Chinese or Japanese. Pass `--lang ch` / `--lang japan` (PaddleOCR's codes — see `references/ocr-ai-languages.md` for the full list; they don't all match ISO codes).
