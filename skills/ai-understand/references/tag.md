...
Full per-model matrix (benchmark accuracy, caption quality, context length, speed) in `references/tag-models.md`.
...
Arbitrary VQA via `--prompt`. Useful prompts are cached in `references/tag-prompts.md` — read it when drafting prompts for object detection, scene analysis, or video narration.
...
For scale beyond ~50 k images, swap the brute-force dot-product for FAISS or Qdrant — the reference doc `references/tag-models.md` has a short recipe, or integrate into your own pipeline.
...
- Read [`references/tag-models.md`](tag-models.md) when picking a specific CLIP / SigLIP / BLIP-2 / LLaVA variant or tuning for VRAM, latency, or benchmark score.
- Read [`references/tag-prompts.md`](tag-prompts.md) when crafting LLaVA prompts for object detection, scene analysis, caption style, or video narration.
- Read [`references/tag-LICENSES.md`](tag-LICENSES.md) before shipping model output commercially or redistributing weights — includes a DROPPED-models list (Florence-2, CogVLM, Gemini, GPT-4V, Claude Vision) and why.

## Gotchas

- **"a photo of X" beats "X".** CLIP / SigLIP classification accuracy jumps 5–10 points when you use full-sentence prompts. The codebase of OpenAI CLIP's eval harness ships 80 prompt templates ("a photo of a small X", "a photo of a large X", etc.) that average to a further 1–3 point gain. See `references/tag-prompts.md`.
...
- **Florence-2 is INTENTIONALLY SKIPPED.** Microsoft dual-licensed it — code is MIT, but the CC-BY-4.0 on the published weights has ambiguity around fine-tuned derivatives. See `references/tag-LICENSES.md` for the details. If/when a clean Apache variant ships, add it back.
...
Solution: Use `--device cpu` (slow but works), or pick a smaller variant (`llava-1.5-7b` is smaller than `-13b`), or enable 4-bit via `bitsandbytes` (see `references/tag-models.md`).
