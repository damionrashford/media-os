...
Absorbed from the former `ffmpeg-lut-grade` skill. Triggers: apply a LUT, apply a .cube file, color grade, cinematic look, film emulation, adjust shadows/midtones/highlights independently, apply a Hald CLUT, fix color cast, match color between clips.
...
Do **not** use this for HDR → SDR tone-mapping (see `references/hdr-color.md`), denoise
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/grade.py lut \
  --input in.mp4 --lut look.cube --output out.mp4 --interp tetrahedral

uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/grade.py haldclut \
  --input in.mp4 --clut clut.png --output out.mp4

uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/grade.py balance \
  --input in.mp4 --output out.mp4 \
  --shadows 0.15,0.05,-0.1 --mids 0,0,0 --highlights -0.1,0,0.1

uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/grade.py selective \
  --input in.mp4 --output out.mp4 --color reds --adjust 0,0,-0.5,0

uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/grade.py match-bt601-to-bt709 \
  --input in.mxf --output out.mp4
```

## Reference docs

- Read [`filters.md`](filters.md) for full option tables
