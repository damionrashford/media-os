...
Absorbed from the former `ffmpeg-ocio-colorpro` skill. Triggers: ACES color management, apply an OCIO config, use a studio/film LUT, embed ICC profiles, convert between ACEScc/ACEScg/sRGB/Rec709/DCI-P3, Resolve-grade post color workflows, match DaVinci Resolve's color pipeline in ffmpeg.
...
If `ocio` is **missing** from the filter list, jump to Gotchas → bake a `.cube` with `ociobake` and apply via the `lut3d` filter (see `references/lut-grade.md`).
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/colorpro.py check
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/colorpro.py aces-to-rec709 --input in.exr --output out.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/colorpro.py transform --input in.mov --output out.mov \
  --pinput "ACEScc" --poutput "Output - Rec.709"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/colorpro.py bake-lut --config $OCIO \
  --pinput "ACES2065-1" --poutput "Output - Rec.709" --output aces_to_709.cube
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/colorpro.py attach-icc --input in.mov --output out.mkv \
...
- For **HDR out of OCIO** (e.g. ACES → HDR10 PQ BT.2020), set both the transform (`poutput="Output - Rec.2020 ST2084"`) AND the container color tags (`-colorspace bt2020nc -color_primaries bt2020 -color_trc smpte2084`) via `references/hdr-color.md`.
...
- [`ocio.md`](ocio.md) — ACES config version matrix, role names, colorspace catalog, ociobake usage, vendor log spaces, recipe book.

## Related skills

- `references/lut-grade.md` — lut3d/haldclut application, ideal fallback when libocio is absent.
- `references/hdr-color.md` — HDR primaries/TRC tagging, tone-mapping.
