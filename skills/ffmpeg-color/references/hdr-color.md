...
Absorbed from the former `ffmpeg-hdr-color` skill. Triggers: convert HDR to SDR, tone-map HDR10, downconvert HDR10 to Rec.709, handle Dolby Vision, convert HLG to SDR, change color primaries or transfer function, make HDR content SDR-playable on phones/web.
...
Or use the helper: `uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py detect --input input.mkv`.
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py detect --input in.mkv
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py hdr-to-sdr --input in.mkv --output out.mp4 --method tonemap --crf 19
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py hdr-to-sdr --input in.mkv --output out.mp4 --method libplacebo
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py hlg-to-sdr --input hlg.mkv --output out.mp4
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ffmpeg-color/scripts/hdrcolor.py bt2020-to-bt709 --input in.mkv --output out.mp4
```

## Reference docs

- Read [`colorspace.md`](colorspace.md) for tone-map operator trade-offs, full `zscale` / `libplacebo` / `colorspace` option catalogs, HDR metadata profiles, and primaries/transfer/matrix lookup tables.
