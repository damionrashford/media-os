...
Full reference: [`config.md`](config.md) has the
...
Full endpoint list: [`api.md`](api.md) or
...
Full auth flow details: `api.md` and
...
- Full `/v3/*` API endpoint catalog with request/response shapes -> [`api.md`](api.md)
- Annotated `mediamtx.yml` with every knob + its default -> [`config.md`](config.md)
