# HDR dynamic-metadata docs lookup
...
- For the execution side (actually running these tools), see `references/dovi-tool.md` + `references/hdr10plus-tool.md`.
...
Read [`references/subcommands.md`](subcommands.md) for the verified subcommand + flag catalog.
...
**Cause:** Wrong tool or wrong page. `convert` is dovi_tool only; `editor` is in both; `x265-encode` isn't a real subcommand (that's a convenience preset in the hdr10plus_tool wrapper).
