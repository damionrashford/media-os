...
- For the doc side (subcommand discovery, flag lookup), use `references/docs-search.md`.
- For HDR10+ (not Dolby Vision), use `references/hdr10plus-tool.md`.
...
Needs Rust (MSRV tracks recent — see docs-search `fetch --page dovi-releases`). Also needs ffmpeg on PATH for the `pipeline` subcommand; mkvmerge (from MKVToolNix) or MP4Box (from GPAC) for remuxing.
...
**Solution:** `dovi_tool convert --help` lists modes. Mode 2 = common "give me 8.1" answer. Or: docs-search `section --page dovi-readme --id convert`.

---

## Reference docs

- Verified dovi_tool subcommand catalog + pipeline chain → `references/subcommands.md`.
- RPU editor JSON schema cheat sheet → `references/edit-schema.md`.
