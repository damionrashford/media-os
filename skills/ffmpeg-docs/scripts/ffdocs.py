...
# dependencies = []
# ///
"""ffdocs.py — Search and fetch official FFmpeg documentation from ffmpeg.org.

Stdlib only (urllib + html.parser). Non-interactive. Caches text-extracted
pages to ~/.cache/ffmpeg-docs/ so repeated lookups are offline-fast.

Usage:
    ffdocs.py list-pages
    ffdocs.py fetch --page ffmpeg-filters
    ffdocs.py search --query "tonemap" [--page ffmpeg-filters]
    ffdocs.py section --page ffmpeg-filters --id tonemap-1
    ffdocs.py index
    ffdocs.py clear-cache

Override cache location with the FFMPEG_DOCS_CACHE env var.
"""
...
UA = "ffmpeg-docs-skill/1.0 (Claude Code Agent Skill)"

PAGES: dict[str, str] = {
    "ffmpeg": "/ffmpeg.html",
    "ffmpeg-all": "/ffmpeg-all.html",
    "ffplay": "/ffplay.html",
    "ffplay-all": "/ffplay-all.html",
    "ffprobe": "/ffprobe.html",
    "ffprobe-all": "/ffprobe-all.html",
...
CACHE_DIR = Path(
    os.environ.get("FFMPEG_DOCS_CACHE", Path.home() / ".cache" / "ffmpeg-docs")
)


class TextExtractor(HTMLParser):
    """Convert ffmpeg.org HTML doc pages into a searchable text form.

    Preserves section headings with anchor IDs (as `[§anchor]`), option tables
    (dt/dd → `**option** — description`), code blocks, and paragraph breaks.
    """
...
def load_or_fetch(page: str, *, no_cache: bool = False, verbose: bool = False) -> str:
    if page not in PAGES:
        raise ValueError(
            f"unknown page: {page!r}. Run `ffdocs.py list-pages` to see valid names."
        )
    cp = cache_path(page)
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for name, path in sorted(PAGES.items()):
        print(f"{name:{width}s}  {BASE}{path}")
    return 0
...
def cmd_fetch(args: argparse.Namespace) -> int:
    ...
    if args.format == "json":
        print(
            json.dumps(
                {"page": args.page, "url": BASE + PAGES[args.page], "text": text},
                indent=2,
            )
        )
    else:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Search and fetch official FFmpeg documentation from ffmpeg.org.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    p1 = sub.add_parser("list-pages", help="list known ffmpeg.org doc pages")
    ...
    p5.add_argument(
        "--id", required=True, help="anchor id (e.g. tonemap-1) or heading keyword"
    )
