# VapourSynth Integration (formerly ffmpeg-vapoursynth)
...
**SVPflow 24 → 60 fps interpolation** (simplified — see `references/vapoursynth-reference.md` for the full recipe):
...
- Keep high-bit-depth (10/16/32-float) through the chain for quality — dithering down only at final output.
