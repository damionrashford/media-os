# DRM & media encryption (HLS AES-128, DASH CENC, ClearKey)
...
- Emits CENC-compliant fmp4 segments and an MPD. For ClearKey playback, append a `<ContentProtection>` ClearKey element (see `references/drm-schemes.md`).
...
- Read [`references/drm-schemes.md`](drm-schemes.md) for HLS vs CENC scheme comparison, MPD ContentProtection templates, browser DRM matrix, ClearKey demo, key rotation strategies, and Shaka/Bento4 handoff when ffmpeg is insufficient.
...
# Patch ContentProtection into the MPD — see references/drm-schemes.md
```

Base64 the hex KID and key per the ClearKey EME spec (`references/drm-schemes.md` has the exact JSON).
...
Solution: inject `ContentProtection` per `references/drm-schemes.md`. `default_KID` must equal your `--kid` formatted with dashes (8-4-4-4-12).
