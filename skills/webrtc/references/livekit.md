...
```bash
# Server
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py install-server --dest ~/bin
# CLI ("lk", formerly "livekit-cli")
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py install-cli --dest ~/bin
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py start --dev
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py mint-token \
...
Token schema (see [`grants.md`](grants.md) for every flag):
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py room-list
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py room-join --room my-room --identity alice
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py load-test --config load-test.yaml
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py egress-start \
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py ingress-start \
  --url ws://localhost:7880 --api-key KEY --api-secret SECRET \
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py install-server --dest ~/bin
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py start --dev        # in one terminal
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/livekit.py load-test \
  --url ws://localhost:7880 --api-key devkey --api-secret secret \
...
- Read [`grants.md`](grants.md) for the full VideoGrant / RoomAdmin grant catalog with per-flag permissions.
- Read [`deployment.md`](deployment.md) for the self-hosted stack (livekit-server, Redis, TURN via bundled Pion or coturn, egress + ingress sidecars, sample YAML).
