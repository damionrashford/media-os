...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/mediasoup.py install
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/mediasoup.py quickstart ~/my-sfu
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/mediasoup.py demo ~/mediasoup-demo
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/mediasoup.py rtp-bridge \
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/mediasoup.py workers
```
...
Read [`transports.md`](transports.md) for full details.
...
- Read [`transports.md`](transports.md) for WebRtcTransport vs PlainTransport vs PipeTransport vs DirectTransport with per-option details.
- Read [`signaling.md`](signaling.md) when choosing a signaling layer — protoo reference plus notes on Socket.IO, raw WebSocket, and HTTP long-polling trade-offs.
