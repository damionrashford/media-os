...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py check
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py list-examples
```
...
Full descriptions in [`examples.md`](examples.md).

## Step 3 — Fetch + build + run

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py fetch-example broadcast --dest ~/pion-broadcast
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py build ~/pion-broadcast
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py run   ~/pion-broadcast
```
...
Read [`primitives.md`](primitives.md) for the full list. Core objects:
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py whip http://localhost:8080/whip --dest ~/pion-whip
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py whep http://localhost:8080/whep --dest ~/pion-whep
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py fetch-example save-to-disk --dest ~/pion-save
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py run ~/pion-save
```

### Example 3 — WHIP + WHEP round-trip

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py whip http://0.0.0.0:8080/whip --dest ~/pion-whip
```

### Example 4 — Insertable streams

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py fetch-example insertable-streams --dest ~/pion-is
uv run ${CLAUDE_PLUGIN_ROOT}/skills/webrtc/scripts/pion.py run ~/pion-is
```
...
- Read [`primitives.md`](primitives.md) when you need the full Pion object + sibling-lib catalog with pkg.go.dev URLs.
- Read [`examples.md`](examples.md) when you need one-paragraph descriptions of every canonical example.
