...
# dependencies = []
# ///
"""ndidocs.py — Search and fetch official NDI documentation.

Covers docs.ndi.video (Vizrt NDI SDK + HDR + ports + release notes + NDI 6),
the SDK landing page at ndi.video/for-developers/ndi-sdk/, and the DistroAV
GitHub README (successor to obs-ndi) as an install/OBS-integration fallback.

Stdlib only (urllib + html.parser). Non-interactive. Caches text-extracted
pages to ~/.cache/ndi-docs/ so repeated lookups are offline-fast.

Usage:
    ndidocs.py list-pages
    ndidocs.py fetch --page sdk-port-numbers
    ndidocs.py search --query "PQ" [--page sdk-hdr]
    ndidocs.py section --page sdk-port-numbers --id discovery
    ndidocs.py index
    ndidocs.py clear-cache

Override cache location with NDI_DOCS_CACHE env var.
"""
...
UA = "ndi-docs-skill/1.0 (Claude Code Agent Skill)"

# name -> full URL. Mix of docs.ndi.video HTML, ndi.video marketing HTML,
# and GitHub raw Markdown for DistroAV (the OBS NDI plugin).
PAGES: dict[str, str] = {
    # Vizrt NDI SDK marketing landing
    "sdk-landing": "https://ndi.video/for-developers/ndi-sdk/",
    # NDI 6 tech page
    "tech-ndi6": "https://ndi.video/tech/ndi6/",
    # docs.ndi.video — developer docs
    "docs-root": "https://docs.ndi.video/",
    "sdk": "https://docs.ndi.video/all/developing-with-ndi/sdk",
    "sdk-release-notes": "https://docs.ndi.video/all/developing-with-ndi/sdk/release-notes",
    "sdk-port-numbers": "https://docs.ndi.video/all/developing-with-ndi/sdk/port-numbers",
    "sdk-hdr": "https://docs.ndi.video/all/developing-with-ndi/sdk/hdr",
    "faq-sdk-vs-advanced": (
        "https://docs.ndi.video/all/faq/sdk/"
        "what-are-the-differences-between-the-ndi-sdk-and-the-ndi-advanced-sdk"
    ),
    # DistroAV (successor to obs-ndi) — GitHub raw Markdown
    "distroav": "https://raw.githubusercontent.com/DistroAV/DistroAV/master/README.md",
}

CACHE_DIR = Path(os.environ.get("NDI_DOCS_CACHE", Path.home() / ".cache" / "ndi-docs"))


class TextExtractor(HTMLParser):
    """Convert NDI docs HTML into searchable text.

    Preserves section headings with anchor IDs (as `[§anchor]`), definition
    lists, code blocks, inline code, and paragraph breaks.
    """
...
def load_or_fetch(page: str, *, no_cache: bool = False, verbose: bool = False) -> str:
    if page not in PAGES:
        raise ValueError(
            f"unknown page: {page!r}. Run `ndidocs.py list-pages` to see valid names."
        )
    cp = cache_path(page)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Search and fetch official NDI documentation.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    p1 = sub.add_parser("list-pages", help="list known NDI doc pages")
    ...
    p5.add_argument(
        "--id",
        required=True,
        help="anchor id (e.g. discovery) or heading keyword",
    )
